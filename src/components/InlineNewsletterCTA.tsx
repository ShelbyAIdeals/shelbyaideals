...
export default function InlineNewsletterCTA({ className }: InlineNewsletterCTAProps) {
  ...
  return (
    <div
      className={clsx(
        'relative my-10 rounded-xl border border-signal-500/20 bg-void-900 p-6 sm:p-8 overflow-hidden',
        className,
      )}
    >
      {/* Background glow */}
      <div className="absolute top-0 right-0 w-40 h-40 bg-signal-500/8 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3" />

      <div className="relative z-10">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-10 h-10 rounded-lg bg-signal-500/10 border border-signal-500/15 text-signal-400 flex items-center justify-center shrink-0">
            <Mail size={20} />
          </div>
          <div>
          ...
          <>
            <form onSubmit={handleSubmit} className="mt-4 flex flex-col sm:flex-row gap-2">
              ...
              <button
                type="submit"
                disabled={loading}
                className="btn-accent text-sm !px-5 !py-2.5 gap-1.5 whitespace-nowrap"
              >
                {loading ? (
