...
function DealCard({ deal, featured = false, t }: { deal: Deal; featured?: boolean; t: (key: string, fallback?: string) => string }) {
  return (
    <div
    ...
    >
      {featured && (
        <div className="absolute -top-3 left-6">
          <span className="inline-flex items-center gap-1 px-3 py-1 text-xs font-bold rounded-full bg-signal-500 text-void-950">
            ...
            {t('deals.featured_deal', 'Featured Deal')}
          </span>
        </div>
      )}
      ...
      <div className="mt-auto flex flex-col gap-2">
        ...
        >
          {t('deals.get_prefix', 'Get')} {deal.tool}
        ...
        {deal.reviewSlug && (
          ...
          >
            {t('deals.read_full_review', 'Read Full Review')}
            <ArrowRight size={12} />
          </Link>
        )}
        <span className="text-[10px] text-void-600 text-center">{t('deals.affiliate_link', 'Affiliate link')}</span>
      </div>
    </div>
  );
}
...
export default function DealsContent({ featured, nonFeatured }: DealsContentProps) {
  ...
  return (
    <div className="container-main pt-48 sm:pt-52 pb-12 sm:pb-16">
      ...
      <ScrollReveal>
        <div className="max-w-2xl mb-14">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-signal-500/10 border border-signal-500/15 text-signal-400 text-xs font-semibold mb-4">
            <Tag size={14} />
            {t('deals.updated_date', 'Updated March 2026')}
          </div>
          <h1 className="text-3xl sm:text-4xl font-bold text-void-50 mb-4 font-heading">
            {t('deals.title', 'AI Tool Deals & Free Trials')}
          </h1>
          <p className="text-lg text-void-400 leading-relaxed">
            {t('deals.description', 'Curated deals on the best AI tools for creators, freelancers, and small teams. Free trials, free tiers, and exclusive discounts — all in one place.')}
          </p>
        </div>
      </ScrollReveal>
      ...
      <ScrollReveal>
        <h2 className="text-xl font-bold text-void-50 font-heading mb-6">
          {t('deals.more_deals', 'More AI Tool Deals')}
        </h2>
      </ScrollReveal>
